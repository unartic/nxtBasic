...
- MODE: can be READ, WRITE or COMMAND
  
**Example:**

    filenumber = OPENFILE("TEST.TXT",READ)  'Opens FILE.TXT for reading
    filenumber = OPENFILE("TEST.TXT",WRITE)  'Opens FILE.TXT for writing
...
# DIRLIST/DIRITEM function
DIRLIST and DIRITEM allow you to read a directory listing based on a filter. Think of DOS"$" on X16, "ls" on linux or "dir" on windows/dos.
...
**Example:**
    ...
    ENTERISO    'enter iso-mode so all filenames are shown as they are
    
    cnt = DIRLIST("*","PRG",LIST_FILES)      'set DIRLIST filter to return all PRG-files

    for i = 1 to cnt                
        PRINT DIRITEM()   'show all these files
    next
